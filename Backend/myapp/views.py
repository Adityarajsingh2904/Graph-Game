...
from myapp.models import Node  # Changed from graphapp to myapp
...
def friendrecs(request):
    if request.method == "POST":
        ...
        return render(request, "myapp/index.html")  # Changed from graphapp to myapp
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for x in results[0][1]:
                ...
                pp.append(x["name"])
            pp2 = tuple(pp)
            return render(request, "myapp/shortest.html", {"path": pp2})  # Changed from graphapp to myapp

        else:
            print("FORM IS INVALID")

    return render(request, "myapp/form_page.html", {"form": form})  # Changed from graphapp to myapp
...
def see_nodes(request):
    ...
    if request.method == "GET":
        ...
        args = {"nodes": nodes}
        return render(request, "myapp/index.html", args)  # Changed from graphapp to myapp
